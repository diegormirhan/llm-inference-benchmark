import atexit, html, json, os, re, signal, subprocess, urllib.request, sys
...
# Proecss manager (one per server, survives)
class ProcessManager:
    ...
    def _open_log(self, path: Path):
        ...
        handle = open(path, "w", buffering = 1, errors = "replace")
    ...
    def start_api(self, engine: str) -> None:
        ...
        cmd = [sys.executable, "main.py", "--engine", engine, "--engine-only"]
    ...
    def stop_api(self) -> None:
        ...
        if proc and proc.poll() is None:
            # SIGTERM para o grupo inteiro: main.py morre, engines.api (filho)
            # recebe o mesmo sinal, seu handler chama engine.shutdown() e
            # os._exit(0). Sem o killpg, main.py (sem handler de SIGTERM)
            # morre e deixa engines.api órfão segurando a porta 8000 + VRAM.
            try:
    ...
    def clear_log(self, path: Path) -> None:
        # Trunca o arquivo de log. O subprocesso que estiver escrevendo nele
        # mantém o seu file descriptor aberto e continua anexando a partir do
        # offset atual, então a operação é segura com API/tester em execução.
        try:
            path.write_text("")
        except OSError:
            pass
...
LOG_LEVEL_RE = re.compile(r"\b(INFO|WARNING|ERROR|DEBUG)\b")
LEVEL_COLORS = {
    "INFO": "#7ee787",
    "WARNING": "#f0b429",
    "ERROR": "#ff6b6b",
...
def _colorize(text: str) -> str:
    # Escapa HTML, destaca o nível (INFO/WARNING/ERROR/DEBUG) e troca \n por <br>.
    safe = html.escape(text, quote=False)
    safe = LOG_LEVEL_RE.sub(
        lambda m: (
            f'<span style="color:{LEVEL_COLORS.get(m.group(1), "#e6edf3")};'
            f'font-weight:600">{m.group(0)}</span>'
        ),
        safe,
    )
    return safe.replace("\n", "<br>")
...
def render_console_log(
    ...
    *,
    ...
    # Console com header (título + contador), fonte 14px, bordas arredondadas
    # e colorização dos níveis. Auto-scroll preservado via JS no fim do HTML.
    body = _colorize(text)
    line_count = text.count("\n") + (0 if text.endswith("\n") else 1) if text else 0
    header = (
        f'<div class="ch">'
        f'<span class="ct">{html.escape(title)}</span>'
        f'<span class="cm">{line_count} lines</span>'
        f'</div>'
        if title
        else ""
    )
    ...
    st.iframe(
        ...
        <style>
            ...
            .wrap {{ display: flex; flex-direction: column; height: {height}px;
            ...
            .ch .ct {{ font-weight: 600; }}
            .ch .cm {{ margin-left: auto; color: #7d8590;
            ...
            .log::-webkit-scrollbar-thumb {{ background: #30363d; border-radius: 5px; }}
...
def load_runs() -> list[dict]:
    # all results_*.json parsed, newest first. Engine/time come from filename.
    runs = []
...
with st.sidebar:
    ...
    def api_status_fragment() -> None:
        ...
        if external:
            st.caption("An API started outside this dashboard in in :8000. Stop it to manage engines here")
    ...
    st.slider("Concurrent Users", min_value = 1, max_value = 100, value = 5, key = "users")
    ...
    def _run_clicked() -> None:
        health = get_api_health()
        # register the results under the engine the API actually loaded
...
with tab_consoles:
    ...
    def consoles_fragment() -> None:
        ...
        with col_api:
            ...
            st.code(pm.api_cmd or f"python main.py --engine {st.session_state.engine_select} --engine-only",
                    language="bash")
            render_console_log(tail_file(API_LOG), height=500, title="api.log")
            st.button(
                "Clear console", key="clear_api", width="stretch",
                on_click=pm.clear_log, args=(API_LOG,),
            )
        ...
        with col_tester:
            ...
            render_console_log(tail_file(TESTER_LOG), height=500, title="tester.log")
            st.button(
                "Clear console", key="clear_tester", width="stretch",
                on_click=pm.clear_log, args=(TESTER_LOG,),
            )
